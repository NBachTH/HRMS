...
Quản lý hợp đồng           (thuế/BH/bậc lương)        lương (PUBLISH)
Chốt kỳ chấm công          ở trạng thái DRAFT         Xem báo cáo chi phí
(AttendancePeriodClose)         │                     Phê duyệt kỳ lương
       │                   Tạo kỳ lương PayrollRun     (PENDING → APPROVED)
       │              →    Tính lương từ dữ liệu  →        ↓
       ▼                   đã chốt kỳ                 FINANCE_ADMIN
[sinh Timesheet]           Trình duyệt                Đánh dấu đã TT
...
Hệ thống chấm công thiết bị nhận diện khuôn mặt sinh ra raw log (ai, lúc mấy giờ, vào hay ra). Để tính lương, cần bản ghi chấm công có cấu trúc (ngày, giờ vào, giờ ra, số giờ làm, đi muộn bao nhiêu). Ba thực thể tham gia, mỗi thực thể một mục đích:

- `CheckinLog`: raw event, bất biến, dùng để debug và audit
- `WorkDay`: bản ghi dẫn xuất theo từng nhân viên × từng ngày — nguồn sự thật duy nhất (single source of truth) cho ngày công, tổng hợp từ các `CheckinLog` trong ngày
- `Timesheet`: bản tổng hợp theo tháng, sinh ra khi **chốt kỳ công** (HR đóng kỳ), gộp toàn bộ `WorkDay` của tháng thành các chỉ số cho tính lương (số ngày công thực tế NCtt, số ngày công chuẩn Nt, số giờ đi muộn…)

> Ghi chú thuật ngữ: trong các đoạn mã minh họa dưới đây, thực thể dẫn xuất theo ngày được gọi là `WorkDay` (phiên bản trước có tên `Attendance`). Nhân viên không được sửa trực tiếp `WorkDay`; muốn điều chỉnh phải nộp **đơn bổ sung chấm công** để Leader/Manager cùng phòng duyệt.

Bài toán kỹ thuật: làm thế nào để tự động chuyển đổi từ raw log sang `WorkDay` mà:
...
Chốt kỳ công — sinh `Timesheet`: cuối tháng, HR thực hiện thao tác **chốt kỳ**. Khi đó hệ thống gộp toàn bộ `WorkDay` của từng nhân viên trong tháng thành một bản ghi `Timesheet` (NCtt — số ngày công thực tế, Nt — số ngày công chuẩn của tháng, tổng giờ đi muộn, số ngày nghỉ có phép…). Đây chính là đầu vào cho khâu tính lương: `Nt` được suy ra tự động từ `Timesheet` thay vì nhập tay, còn `NCtt` lấy trực tiếp từ các `WorkDay` đã chốt. Sau khi chốt, các `WorkDay` của kỳ bị khóa; mọi điều chỉnh phải đi qua đơn bổ sung chấm công có phê duyệt, giữ cho dữ liệu tính lương luôn truy vết được.
...
baseGross = [(Lhq × HStb) + Li + HTi] × (NCtt / Nt)

Trong đó:
  Lhq    = baseSalary từ Contract hiệu lực trong tháng
  HStb   = (HS1 + HS2) / 2          (hai hệ số đánh giá, trung bình cộng)
  HS1    = hệ số hiệu suất do cấp trên chấm (A=1.04, B=1.00, C=0.98);
              mặc định B=1.00 nếu chưa chấm. Với NHÂN VIÊN/LEADER: lấy
              điểm cấp trên trực tiếp chấm. Với MANAGER: lấy trung bình
              HS1 của đơn vị mình quản lý. Với DIRECTOR: lấy trung bình
              HS1 của toàn công ty.
  HS2    = hệ số chuyên cần, suy ra tự động từ WorkDay/Timesheet
              (theo quy chế 01/2020/QC-VTI):
              A (1.04): không vi phạm chấm công và không nghỉ trong tháng
              B (1.02): không vi phạm nhưng có nghỉ ít nhất một lần
              C (1.00): có ít nhất một vi phạm (đi muộn/về sớm, < 8h,
                        vắng không thông báo)
              Với MANAGER: trung bình HS2 của đơn vị; với DIRECTOR:
              trung bình HS2 của toàn công ty.
  Li     = positionAllowance (phụ cấp chức vụ)
  HTi    = otherAllowances (phụ cấp khác)
  NCtt   = paidDays — số ngày công thực tế, lấy từ WorkDay đã chốt
  Nt     = số ngày công chuẩn của tháng, suy ra tự động từ Timesheet
              (không còn nhập tay)
```
...
Thuế bậc được đọc từ bảng cấu hình lương có hiệu lực-theo-ngày (typed, effective-dated, append-only — nhóm `PIT`) đang hiệu lực trong kỳ. Finance tạo phiên bản cấu hình mới ở trạng thái DRAFT, Director phê duyệt (PUBLISH) thì phiên bản đó được áp dụng theo ngày hiệu lực — cập nhật biểu thuế khi Nhà nước thay đổi chính sách mà không cần sửa code (xem 5.1).
